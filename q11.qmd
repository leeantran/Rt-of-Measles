...
df_q11 <- filter(df, qh == "Quận 11")
```

```{r}
df_convert <- df_q11 %>% group_by(dates) %>% 
  summarise(I = n())
...
-   Dữ liệu của quận bắt đầu có số ca liên tục từ sau ngày **05/08/2024**, do đó sẽ bắt đầu ước tính ***R~t~*** từ ngày **05/08/2024.**

```{r}
df_filter <- filter(df_complete, dates >= "2024-08-05")
```
...
p_hist <- ggplot(df_filter, aes(x = dates, y = I)) +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8)) + 
    scale_x_date(date_labels = "%b %d", date_breaks = "1 week", limits = c(ymd("2024-08-05"), ymd("2024-09-14")))
...
p_hist / p_rt
```
-   Do để tính ***R~t~*** chúng ta cần khoảng thời gian thế hệ (Serial interval) là 14 ngày, do đó nếu dữ liệu để tính bắt đầu từ **05/08/2024** thì ***R~t~*** sẽ được tính từ ngày **19/08/2024**. Giá trị ***R~t~*** vào ngày đầu tiên là **3.16 (KTC 95%: 0,86 - 6.94)**, sau 5 ngày thì **R~t~ giảm còn 2.41 (KTC 95%: 0.78 - 4.94)**.
...
p_rt <- ggplot(df_rt, aes(x = dates)) +
    ...
    annotate("text", x = ymd("2024-09-01"), y = 7.5, label = "Chiến dịch tiêm chủng", 
             size = 5, hjust = -0.1, color = "black", fontface = "bold") +
    annotate("text", x = ymd("2024-08-19"), y = 7.5, label = "Chưa có can thiệp", 
             size = 5, hjust = -0.1, color = "black", fontface = "bold") +
    annotate("rect", xmin = ymd("2024-08-31"), xmax = ymd("2024-09-14"), ymin = 7, ymax = 8,
             alpha = 0.15, fill = "lightblue", color = "black") +
    annotate("rect", xmin = ymd("2024-08-12"), xmax = ymd("2024-08-31"), ymin = 7, ymax = 8, 
             alpha = 0.15, fill = "lightblue", color = "black")


p_hist / p_rt
```
-   Từ ngày 31/08/2024, bắt đầu triển khai "Chiến dịch tiêm chủng" trên cộng đồng và trên đối tượng trẻ từ 1 - 10 tuổi. Có thể thấy được sau khi thực hiện chiến dịch được 1 tuần thì số R~t~ đi ngang và giảm từ ngày 31/08 với ***R~t~ = 6.08*** xuống còn ***R~t~ = 1.60*** vào ngày 10/09. Đến thời điểm hiện tại, chúng ta chưa thấy được rõ hiệu quả của chiến dịch tiêm chủng mang lại có thể do: (1) thời gian tạo kháng thể của vắc xin cần từ 2 - 4 tuần hoặc (2) chiến dịch tiêm chủng ở huyện Củ Chi có thể chưa tốt.
