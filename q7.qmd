...
```{r}
# Lọc dữ liệu từ 30/05/2024 - end và lọc ra quận 7
df_q7 <- filter(df, qh == "Quận 7")
```

```{r}
df_convert <- df_q7 %>% group_by(dates) %>% 
  summarise(I = n())
...
-   Dữ liệu của quận bắt đầu có số ca liên tục từ sau ngày **07/08/2024**, do đó sẽ bắt đầu ước tính ***R~t~*** từ ngày **07/08/2024.**

```{r}
df_filter <- filter(df_complete, dates >= "2024-08-07")
```
...
p_hist <- ggplot(df_filter, aes(x = dates, y = I)) +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8)) + 
    scale_x_date(date_labels = "%b %d", date_breaks = "1 week", limits = c(ymd("2024-08-07"), ymd("2024-09-14")))
...
p_rt <- ggplot(df_rt, aes(x = dates)) +
  ...
  labs(x = "Day", y = "Rt", title = "19/08/2024 - 14/09/2024") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8)) +
  scale_x_date(date_labels = "%b %d", date_breaks = "1 week", limits = c(ymd("2024-08-07"), ymd("2024-09-14"))) +
...
p_hist / p_rt
```
-   Do để tính ***R~t~*** chúng ta cần khoảng thời gian thế hệ (Serial interval) là 14 ngày, do đó nếu dữ liệu để tính bắt đầu từ **07/08/2024** thì ***R~t~*** sẽ được tính từ ngày **21/08/2024**. Giá trị ***R~t~*** vào ngày đầu tiên là **3.01 (KTC 95%: 0,82 - 6.61)**, sau 4 ngày thì **R~t~ giảm còn 1.87 (KTC 95%: 0.61 - 3.84)**.
...
p_rt <- ggplot(df_rt, aes(x = dates)) +
    ...
    geom_segment(aes(x = as.Date("2024-08-31"), xend = as.Date("2024-08-31"), y = 7, yend = 0),
    ...
    annotate("text", x = ymd("2024-09-01"), y = 6.5, label = "Chiến dịch tiêm chủng", 
             size = 5, hjust = -0.1, color = "black", fontface = "bold") +
    annotate("text", x = ymd("2024-08-19"), y = 6.5, label = "Chưa có can thiệp", 
             size = 5, hjust = -0.1, color = "black", fontface = "bold") +
    annotate("rect", xmin = ymd("2024-08-31"), xmax = ymd("2024-09-14"), ymin = 6, ymax = 7,
             alpha = 0.15, fill = "lightblue", color = "black") +
    annotate("rect", xmin = ymd("2024-08-12"), xmax = ymd("2024-08-31"), ymin = 6, ymax = 7, 
             alpha = 0.15, fill = "lightblue", color = "black")


p_hist / p_rt
```
-   Từ ngày 31/08/2024, bắt đầu triển khai "Chiến dịch tiêm chủng" trên cộng đồng và trên đối tượng trẻ từ 1 - 10 tuổi. Có thể thấy được R~t~ bắt đầu đi ngang từ ngày 27/08 với ***R~t~ = 1.51*** đến ngày 02/09 (***R~t~ = 1.43***, tuy nhiên sau đó lại tăng lên vào ngày 03/09 (***R~t~ = 2.16***) và đi ngang đến 10/09. Đến thời điểm hiện tại, chúng ta chưa thấy được rõ hiệu quả của chiến dịch tiêm chủng mang lại có thể do: (1) thời gian tạo kháng thể của vắc xin cần từ 2 - 4 tuần hoặc (2) chiến dịch tiêm chủng ở huyện Củ Chi có thể chưa tốt.
