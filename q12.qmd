...
```{r}
# Lọc dữ liệu từ 30/05/2024 - end và lọc ra quận 12
df_q12 <- filter(df, qh == "Quận 12")
```

```{r}
df_convert <- df_q12 %>% group_by(dates) %>% 
  summarise(I = n())
...
-   Dữ liệu của quận bắt đầu có số ca liên tục từ sau ngày **08/07/2024**, do đó sẽ bắt đầu ước tính ***R~t~*** từ ngày **08/07/2024.**

```{r}
df_filter <- filter(df_complete, dates >= "2024-07-08")
```
...
p_hist <- ggplot(df_filter, aes(x = dates, y = I)) +
    ...
    theme_minimal() +
    theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8)) + 
    scale_x_date(date_labels = "%b %d", date_breaks = "1 week", limits = c(ymd("2024-07-08"), ymd("2024-09-14")))
...
p_rt <- ggplot(df_rt, aes(x = dates)) +
  ...
  labs(x = "Day", y = "Rt", title = "22/07/2024 - 14/09/2024") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 60, hjust = 1, size = 8)) +
  scale_x_date(date_labels = "%b %d", date_breaks = "1 week", limits = c(ymd("2024-07-08"), ymd("2024-09-14"))) +
...
p_hist / p_rt
```
-   Do để tính ***R~t~*** chúng ta cần khoảng thời gian thế hệ (Serial interval) là 14 ngày, do đó nếu dữ liệu để tính bắt đầu từ **08/07/2024** thì ***R~t~*** sẽ được tính từ ngày **22/07/2024**. Giá trị ***R~t~*** vào ngày đầu tiên là **4,9 (KTC 95%: 1,01 - 11,9)**, sau 9 ngày thì **R~t~ giảm còn 2.3 (KTC 95%: 0.6 - 5.1)** và sau đó giảm dần gần tăng lên cao lại vào ngày **20/08** với ***R~t~ = 3.9***. Sau đó giảm mạnh xuống gần bằng 1.
...
p_rt <- ggplot(df_rt, aes(x = dates)) +
    ...
    geom_segment(aes(x = as.Date("2024-08-31"), xend = as.Date("2024-08-31"), y = 4.5, yend = 0),
    ...
    scale_y_continuous(limits = c(0, 4.5), breaks = seq(0, 5.5, by = 1)) +
    annotate("text", x = ymd("2024-09-01"), y = 4.25, label = "Chiến dịch tiêm chủng", 
             size = 5, hjust = -0.1, color = "black", fontface = "bold") +
    annotate("text", x = ymd("2024-08-19"), y = 4.25, label = "Chưa có can thiệp", 
             size = 5, hjust = -0.1, color = "black", fontface = "bold") +
    annotate("rect", xmin = ymd("2024-08-31"), xmax = ymd("2024-09-14"), ymin = 4, ymax = 4.5,
             alpha = 0.15, fill = "lightblue", color = "black") +
    annotate("rect", xmin = ymd("2024-08-12"), xmax = ymd("2024-08-31"), ymin = 4, ymax = 4.5, 
             alpha = 0.15, fill = "lightblue", color = "black")
...
-   Từ 12/08/2024, R~t~ của quận 12 có xu hướng tăng trở lại, cao nhất là ngày 20/08 với R~t~ = 3.9, tuy nhiên sau đó R~t~ lại giảm và bắt đầu đi ngang từ 31/08/2024.

-   Từ ngày 31/08/2024, bắt đầu triển khai "Chiến dịch tiêm chủng" trên cộng đồng và trên đối tượng trẻ từ 1 - 10 tuổi. Có thể thấy được sau khi thực hiện chiến dịch được 1 tuần thì số R~t~ bắt đầu giảm gần bằng 1. Tuy nhiên, R~t~ tăng trở lại vào ngày 10/09 (***R~t~ = 1.4***), và giảm còn **1.02** vào ngày 14/09/2024. Đến thời điểm hiện tại, chúng ta chưa thấy được rõ hiệu quả của chiến dịch tiêm chủng mang lại có thể do: (1) thời gian tạo kháng thể của vắc xin cần từ 2 - 4 tuần hoặc (2) chiến dịch tiêm chủng ở quận 12 có thể chưa tốt.
